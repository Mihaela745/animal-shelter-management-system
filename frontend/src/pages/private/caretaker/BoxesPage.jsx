...
export default function BoxesPage() {
  ...
  const { myProfile, loading: profileLoading } = useSelector((s) => s.staff);
  const {
    staffBoxes,
    responsiblesByBoxId,
  ...
  const filteredBoxes = useMemo(
    ...
    [filterSpecies, search, staffBoxes],
  );
  ...
  const handleOpenAnimals = (boxItem) => {
    navigate(`/staff/boxes/${boxItem.id}/animals`);
  };

  const isLoading = profileLoading || speciesLoading || responsiblesLoading;
      ...
      >
        <Box>
          ...
          <Typography sx={{ fontSize: "0.85rem", color: "#aaa", ml: "52px" }}>
            {staffBoxes.length} boxe atribuite pentru ingrijire
          </Typography>
        </Box>
      </Box>
      ...
      {isLoading ? (
        ...
        >
          ...
          <Typography sx={{ fontSize: "0.82rem", color: "#ccc", mt: 0.5 }}>
            Cand un manager iti asigneaza boxe, ele vor aparea aici
          </Typography>
        </Box>
      ) : (
        <Box

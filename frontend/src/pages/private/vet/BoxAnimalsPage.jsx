...
export default function VetBoxAnimalsPage() {
  const { id } = useParams();
  const navigate = useNavigate();
  const dispatch = useDispatch();

  const { animals, loading } = useSelector((state) => state.animals);
  const { selectedBox, loading: boxesLoading } = useSelector((state) => state.boxes);
      ...
      {loading || boxesLoading ? (
        ...
        >
          ...
          <Typography sx={{ fontSize: "0.82rem", color: "#ccc", mt: 0.5 }}>
            Poti reveni la lista de boxe pentru alte boxe
          </Typography>
        </Box>
      ) : (
        <Box

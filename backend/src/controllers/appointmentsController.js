...
export const controller = {
  ...
  getAllAppointments: async (req, res) => {
    try {
      ...
      if (!appointment)
        return res.status(404).send(`Couldn't find the appointment`);
  ...
  updateAppointment: async (req, res) => {
    try {
      ...
      const updatedAppointment = await Appointments.findByPk(appointmentId);
  ...
  deleteAppointment: async (req, res) => {
    try {
      ...
      const deletedAppointment = await Appointments.destroy({where:{id:appointment}});
